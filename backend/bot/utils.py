...
from backend.bot.states import MailingCreate
from backend.bot.mailingconstructor import MailingConstructor
...
async def make_safe_request(method: Callable, *args, **kwargs) -> Optional[Response]:
    try:
        ...
        if response.status_code not in [200, 201, 400, 404, 409, 422]:
            logger.debug(
...
def build_constructor_keyboard(
    ...
    exit_save_buttons = [("Отправить", "save_mailing"), ("Выйти", "exit_constructor")]
    delete_buttons = []
...
async def get_constructor(state: FSMContext) -> MailingConstructor:
    ...
    return MailingConstructor.from_dict(redis_data["constructor"])
